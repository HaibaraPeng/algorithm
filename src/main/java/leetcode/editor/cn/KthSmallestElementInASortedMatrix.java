...
// 
//
// Related Topics 数组 二分查找 矩阵 排序 堆（优先队列） 👍 1087 👎 0
...
public class KthSmallestElementInASortedMatrix {
    ...
    class Solution {
        public int kthSmallest(int[][] matrix, int k) {
            ...
            for (int i = 0; i < k; i++) {
                int minIndex = 0;
                while (arr[minIndex] == matrix.length) {
                    minIndex++;
                }
                int min = matrix[minIndex][arr[minIndex]];
                for (int j = minIndex + 1; j < matrix.length; j++) {
                    int val = matrix[j][arr[j]];
                    if (val < min) {
                        min = val;
                        minIndex = j;
                    }
